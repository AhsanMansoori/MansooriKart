  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <Button className={tab === 'orders' ? '' : 'bg-muted text-foreground hover:bg-border'} onClick={() => setTab('orders')}>
          Customer Orders
        </Button>
        <Button className={tab === 'fulfillments' ? '' : 'bg-muted text-foreground hover:bg-border'} onClick={() => setTab('fulfillments')}>
        ...
        <div className="grid gap-3 md:grid-cols-[200px_1fr_auto]">
          {tab === 'orders' ? (
            ...
            <Select value={fulfillmentStatus} onChange={event => setFulfillmentStatus(event.target.value)}>
              ...
              {['PENDING', 'SENT_TO_SUPPLIER', 'SUPPLIER_CONFIRMED', 'SHIPPED', 'DELIVERED', 'FAILED', 'CANCELLED'].map(value => (
                <option key={value}>{value}</option>
              ))}
            </Select>
          )}
          <Input value={search} onChange={event => setSearch(event.target.value)} placeholder="Exact order number" />
          <Button onClick={() => (tab === 'orders' ? void loadOrders() : void loadFulfillments())}>
      ...
      {busy === 'load' ? (
        ...
        !orders.length ? (
          <EmptyState title="No orders found">Try another status or order number.</EmptyState>
        ) : (
                  ...
                  {orders.map(order => (
                    <tr key={order._id}>
                      <td className="px-4 py-4">
                        <button className="font-bold text-left hover:underline" onClick={() => void openOrder(order._id)}>
                          {order.orderNumber}
                        </button>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <Select
                          ...
                          onChange={event => void changeOrderStatus(order, event.target.value as AdminOrder['orderStatus'])}
                      ...
                      <td className="px-4 py-4">
                        <Select
                          ...
                          onChange={event => void changePayment(order, event.target.value as AdminOrder['paymentStatus'])}
                          disabled={busy === order._id}
                        >
                          {['PENDING', 'UNPAID', 'PAID', 'FAILED', 'REFUNDED', 'PARTIALLY_REFUNDED'].map(value => (
                            <option key={value}>{value}</option>
                          ))}
                        </Select>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <Button className="bg-muted text-foreground hover:bg-border" onClick={() => void openOrder(order._id)} disabled={busy === order._id}>
                          Details
                        </Button>
                        {order.orderStatus !== 'CANCELLED' && order.orderStatus !== 'DELIVERED' ? (
                          <Button className="ml-2 bg-destructive text-destructive-foreground" onClick={() => void cancel(order)} disabled={busy === order._id}>
                            Cancel
                          </Button>
                        ) : null}
                      </td>
        ...
        <EmptyState title="No dropship fulfillments found">Fulfillment records are created from dropship order lines at checkout.</EmptyState>
      ) : (
        <Card className="overflow-hidden">
          <div className="overflow-x-auto">
                ...
                {fulfillments.map(item => (
                  <tr key={item.id}>
                    <td className="px-4 py-4">
                      ...
                      <div className="text-xs text-muted-foreground">Lines: {item.orderItems.join(', ') || '—'}</div>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      {item.supplierTrackingNumber || '—'}
                      {item.carrier ? <div className="text-xs text-muted-foreground">{item.carrier}</div> : null}
                    </td>
                    <td className="px-4 py-4">
                      {item.allowedTransitions?.length ? (
                        <Select
                          defaultValue=""
                          onChange={event => {
                            if (event.target.value) void advanceFulfillment(item, event.target.value);
                          }}
        ...
        <Card className="p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {customerLabel(selectedOrder)} · {money(selectedOrder.total, selectedOrder.currency)}
              </p>
            </div>
          ...
          <div className="mt-6">
            <h3 className="flex items-center gap-2 font-bold">
              <Truck size={18} aria-hidden="true" /> Dropship obligations
            </h3>
            {!selectedOrder.dropshipFulfillments?.length ? (
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-2">
                {selectedOrder.dropshipFulfillments.map(item => (
                  <div key={item.id} className="rounded-mk border border-border p-4">
