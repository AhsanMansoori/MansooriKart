...
describe('<Register />', () => {
  const fillMatchingRegistration = () => {
    ...
    const [password, confirmPassword] = screen.getAllByLabelText(/password/i, { selector: 'input' });
    fireEvent.change(password, { target: { value: 'pw' } });
    fireEvent.change(confirmPassword, { target: { value: 'pw' } });
  };
  ...
  it('on success stores token & redirects', async () => {
    ...
    fillMatchingRegistration();

    fireEvent.click(screen.getByRole('button', { name: /register/i }));

    await waitFor(() => expect(apiClient.post).toHaveBeenCalledTimes(1));
    await waitFor(() => expect(localStorage.getItem('mansoorikart_access_token')).toBe('tok123'));
