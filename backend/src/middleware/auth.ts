...
export async function requireAuth(request: Request, response: Response, next: NextFunction): Promise<void> {
  ...
  try {
    const payload = jwt.verify(token, getConfig().jwtSecret) as { sub?: string; role?: 'CUSTOMER' | 'SUPER_ADMIN'; ver?: number };
    if (!payload.sub) return void sendFailure(response, 401, 'AUTH_UNAUTHORIZED', 'Authentication is required.', request.requestId);
    const user = await User.findById(payload.sub).select('_id role status +sessionVersion').lean();
    if (!user || user.status !== 'ACTIVE' || (payload.ver ?? 0) !== (user.sessionVersion ?? 0))
      return void sendFailure(response, 401, 'AUTH_UNAUTHORIZED', 'Authentication is required.', request.requestId);
