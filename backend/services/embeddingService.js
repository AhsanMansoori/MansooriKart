...
const embedText = async (text, options = {}) => {
  ...
  const response = await getEmbeddingModel().embedContent(buildEmbedRequest(trimmedText, options));
  const values = response?.embedding?.values;

  if (!Array.isArray(values) || values.length !== EMBEDDING_DIMENSION) {
    throw new Error(`Gemini embedding length mismatch: expected ${EMBEDDING_DIMENSION}, received ${values?.length || 0}`);
  }
