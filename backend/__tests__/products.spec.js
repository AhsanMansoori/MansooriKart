...
describe('Products API', () => {
  ...
  describe('PUT /api/products/:id/rating', () => {
    it('200 → updates rating when product exists', async () => {
      ...
      const res = await request(app).put('/api/products/z1/rating').send({ rating: 5 });
    ...
    it('404 → product not found', async () => {
      Product.findById.mockResolvedValue(null);
      const res = await request(app).put('/api/products/nope/rating').send({ rating: 1 });
      expect(res.status).toBe(404);
      expect(res.text).toBe('Product not found');
    });
    ...
    it('500 → server error', async () => {
      ...
      const res = await request(app).put('/api/products/err/rating').send({ rating: 2 });
      expect(res.status).toBe(500);
      expect(res.text).toBe('Server error');
    });
  });
