...
4. **Write Tests** (`backend/__tests__/products.spec.js`):
   ...
   describe('POST /api/products/:id/reviews', () => {
     it('should add a review', async () => {
       const res = await request(app).post(`/api/products/${productId}/reviews`).send({ rating: 5, comment: 'Great!', userName: 'John' });
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const sortedProducts = mongoIds.map(id => idToProduct.get(id)).filter(Boolean);
...
router.get('/', async (req, res) => {
  ...
  const [products, total] = await Promise.all([Product.find().skip(skip).limit(limit).lean(), Product.countDocuments()]);
...
useEffect(() => {
  axios.get(`/api/products?page=${page}&limit=20`).then(res => {
...
return (
  <>
    ...
    <Pagination count={pagination.pages} page={page} onChange={(e, val) => setPage(val)} />
  </>
);
```
