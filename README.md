...
> **Repository transition notice.** This README preserves substantial upstream Fusion documentation for historical reference. MansooriKart's current workspace, container, CI, and deployment status is documented in [`docs/deployment/README.md`](docs/deployment/README.md). No active Kubernetes deployment or selected hosting provider is established by this repository.
...
The repository is an npm workspace with `frontend/` and `backend/` packages. From the repository root:

```bash
npm install
npm run dev:frontend   # Vite frontend at http://localhost:5173
npm run dev:backend    # TypeScript v1 backend watch process
npm run test:frontend
npm run build:frontend
npm run typecheck
```

Backend commands remain available through the workspace (`npm run test:backend`, `npm run test:v1`, and `npm run typecheck:backend`). The legacy API remains supported during the frontend's staged `/api/v1` migration.
...
<p align="center">
  ...
  <a href="https://nodejs.org/">
    <img src="https://img.shields.io/badge/Node.js-26.x-339933?logo=node.js&logoColor=white" alt="Node.js badge" />
  </a>
...
The following upstream Fusion links are historical references only; they are not verified MansooriKart deployment targets. MansooriKart does not currently claim a live Vercel, Render, or Kubernetes deployment.
...
- **Frontend:**
  ...
  - Custom notification provider for toast notifications
...
- Node.js 26.x (with npm). Verified in Phase 1 with Node.js v26.5.0 and npm 11.17.0.
...
4. Set up the frontend:
   ...
   - Start the frontend development server:
     ...
     ```
     > [!TIP]
     > Vite prints the local development URL when the server starts (normally `http://localhost:5173`).

## Running the Application

- Open the local URL printed by Vite (normally `http://localhost:5173`) to view the application.
...
4. _(Optional)_ **Build and query the FAISS index**:
   ```bash
   cd backend
...
MansooriKart provides workspace-aware Docker definitions for reproducible frontend and backend images. No hosting provider or active Kubernetes deployment is configured. Current container, Compose, CI, health-check, and environment-name guidance is in [docs/deployment/README.md](docs/deployment/README.md); the former Fusion deployment guide is historical reference only.
