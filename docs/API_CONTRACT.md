...
Reports perform no writes and aggregate the live collections — there is no reporting warehouse, no materialised summary collection, and no rollup job to fall out of sync. **The Import/Export ERP module is deferred**, and the index says so machine-readably: `export: { available: false, reason: … }`. There is no CSV endpoint, no XLSX generation, and no scheduled delivery.
