...
> **Historical archive — not current MansooriKart production architecture.** Any Fusion/Jenkins/Kubernetes sections are retained as reference only. Kubernetes is archived, no active cluster is evidenced, and current container/CI guidance is in [`docs/deployment/README.md`](docs/deployment/README.md).
