...
> **Historical archive — not current MansooriKart deployment guidance.** The referenced Jenkins, Kubernetes, blue-green, and canary assets have been archived under `infrastructure/`. MansooriKart has no active Kubernetes deployment or selected production hosting provider. Use [`docs/deployment/README.md`](docs/deployment/README.md) for current container and CI guidance.
